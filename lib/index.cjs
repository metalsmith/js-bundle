...
function es5PluginInit() {
  return function es5Plugin(files, ms, done) {
    ...
    const transforms = [];
    const options = core.loadOptions({
    ...
    Object.values(files).forEach(file => {
      if (file.__babelPostProcess) {
        transforms.push(new Promise((resolve, reject) => {
          core.transform(file.contents.toString(), options, (err, result) => {
            if (err) reject(err);
...
function normalizeOptions(options = {}, metalsmith, debug) {
  ...
  const overwrites = {
    entryPoints,
    absWorkingDir: msDir,
    outdir: path.relative(msDir, metalsmith.destination()),
    write: false,
    metafile: true,
    define
  };
...
function initJsBundle(options = {}) {
  ...
  return function jsBundle(files, metalsmith, done) {
    ...
    const sourceRelPath = path.relative(metalsmith.directory(), src);
    esbuild.build(normalizedOptions).then(result => {
      ...
      debug('Finished processing files %O', result.outputFiles.map(o => path.relative(dest, o.path)));
      ...
      Object.values(result.metafile.outputs).forEach(o => {
        if (o.inputs) {
          metalsmith.match(`${sourceRelPath}/**`, Object.keys(o.inputs)).forEach(input => {
            delete files[path.relative(src, metalsmith.path(input))];
          });
        }
      });
      ...
      result.outputFiles.forEach(file => {
        ...
        const destPath = path.relative(dest, file.path);
        files[destPath] = {
          contents: Buffer.from(file.contents.buffer)
        };
        if (babelPostProcess && path.extname(destPath) === '.js') {
